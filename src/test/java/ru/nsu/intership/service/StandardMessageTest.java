...
public class StandardMessageTest {
    ...
    public void testSendOnWrongUrl() throws IOException {
        ...
        verify(log, times(1)).error("Message " + message.getMessage() + " sending failed for" + url);
    ...
    public void testSendBadRequestUrl() throws IOException {
        ...
        sender.send(message, url);
        verify(log, times(0)).info(logInfo);
        verify(log, times(1)).warn("Bad request when message " + message.getMessage() + " send for" + url);
